...
module.exports = function() {
    ...
    // Direct url link to a specific person
    ...
    _helpers.ifEquals = function(arg1, arg2, options) {
        return (arg1 == arg2) ? options.fn(this) : options.inverse(this);
    }
    ...
    _helpers.blogPaginationNavigation = function(pages, currentPage, totalPages, options) {
        var html = '';
        ...
        _.each(pages, function(page, ctr) {
            ...
            var blogPageUrl = _helpers.blogPageUrl(page);
            // wrapup the html
    ...
    _helpers.blogPaginationPreviousUrl = function(previousPage, totalPages) {
    ...
    _helpers.blogPaginationNextUrl = function(nextPage, totalPages) {
        if (nextPage === false) {
            nextPage = totalPages;
        }
    ...
    // people
    // used for pagination urls on blog or people
    _helpers.peoplePageUrl = function(pageNumber, options) {
        return 'people?page=' + pageNumber;
    };
    ...
    _helpers.peoplePaginationNavigation = function(pages, currentPage, totalPages, options) {
        var html = '';
        ...
        _.each(pages, function(page, ctr) {
            ...
            // get the pageUrl using the integer value
            var peoplePageUrl = _helpers.peoplePageUrl(page);
            // wrapup the html
            html += '<li' + liClass + '>' + linkTemplate({
                url: peoplePageUrl,
                text: pageText
    ...
    // special helper to ensure that we always have a valid page url set even if
    // the link is disabled, will default to page 1
    _helpers.peoplePaginationPreviousUrl = function(previousPage, totalPages) {
        if (previousPage === false) {
            previousPage = 1;
        }
        return _helpers.peoplePageUrl(previousPage);
    };

    // special helper to ensure that we always have a valid next page url set
    // even if the link is disabled, will default to totalPages
    _helpers.peoplePaginationNextUrl = function(nextPage, totalPages) {
        if (nextPage === false) {
            nextPage = totalPages;
        }
        return _helpers.peoplePageUrl(nextPage);
    };
